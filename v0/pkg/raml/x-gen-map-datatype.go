...
type DataTypeMap interface {
	...
	Put(key string, value DataType) DataTypeMap

	PutNonNil(key string, value DataType) DataTypeMap
	...
	Replace(key string, value DataType) DataType
	...
	// This method returns the value previously stored at
	// `key` if such a value existed.
	ReplaceOrPut(key string, value DataType) DataType
	...
	// map.
	At(index uint) (key option.String, value DataType)
	...
	Delete(key string) DataType

	ForEach(fn func(k string, v DataType))
}
