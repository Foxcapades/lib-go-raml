...
// DateOnlyExampleMap generated @ 2020-05-21T01:49:31.367162698-04:00
type DateOnlyExampleMap interface {
	...
	Put(key string, value DateOnlyExample) DateOnlyExampleMap

	PutNonNil(key string, value DateOnlyExample) DateOnlyExampleMap
	...
	Replace(key string, value DateOnlyExample) DateOnlyExample
	...
	// This method returns the value previously stored at
	// `key` if such a value existed.
	ReplaceOrPut(key string, value DateOnlyExample) DateOnlyExample

	// Get returns the value stored at the given key (if it
	// exists) or nil if no such key was found.
	Get(key string) DateOnlyExample
	...
	// map.
	At(index uint) (key option.String, value DateOnlyExample)
	...
	Delete(key string) DateOnlyExample

	ForEach(fn func(k string, v DateOnlyExample))
}
