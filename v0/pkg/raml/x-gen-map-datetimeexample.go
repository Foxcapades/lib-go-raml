...
// DatetimeExampleMap generated @ 2020-05-20T20:54:25.054891636-04:00
type DatetimeExampleMap interface {
	...
	Put(key string, value DatetimeExample) DatetimeExampleMap

	PutNonNil(key string, value DatetimeExample) DatetimeExampleMap
	...
	Replace(key string, value DatetimeExample) DatetimeExample
	...
	// This method returns the value previously stored at
	// `key` if such a value existed.
	ReplaceOrPut(key string, value DatetimeExample) DatetimeExample

	// Get returns the value stored at the given key (if it
	// exists) or nil if no such key was found.
	Get(key string) DatetimeExample
	...
	// map.
	At(index uint) (key option.String, value DatetimeExample)
	...
	Delete(key string) DatetimeExample

	ForEach(fn func(k string, v DatetimeExample))
}
